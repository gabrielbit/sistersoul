<?php
/**
 * JFUploader 2.16.x Freeware - Plugin for Joomla 1.5.x
 *
...
global $mainframe,$prefix_path;
$mainframe->registerEvent( 'onPrepareContent', 'botJFUploader' );

function botJFUploader( &$row, &$params, $page=0 ) 
{
   global $mainframe,$prefix_path;
     ...
     JPlugin::loadLanguage( 'com_jfuploader' );
      ...
      echo '<!-- JFU type: \'' . $selector . '\' id: \'' .  $id . '\' -->';   
 	  ...
 	  if (isset($selector) && isset($id)) { 	  
          ...
          } else if (securityTokenIsValid($selector, $id, $securitytoken))  {
                 $user	=& JFactory::getUser();
      		  $old_error = error_reporting(0);
      		  $myId = JFUHelper::getProfileId($selector, $id, $user);
      		  error_reporting($old_error);
              if ($myId >=0) {
        		     $mycode = showFlashPlugin($myId, $twgpath);
        		  } else {
        		     $mycode = HTML_joomla_flash_uploader::wrongId($id, true); 
              }
          } else {
...
function showFlashPlugin($id, $twgpath) {
   global $prefix_path;
	 ...
	 if (!$row->resize_show) { // no profile found or no id!
       ...
       $thumbnailflash = '';
       if ($twgpath != '') {
         $thumbnailflash = getFlashContent($twgpath, $uploadfolder,$use_js_include);
       }

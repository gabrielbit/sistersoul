...
class agGallery extends agHelper{
    ...
    function writePagination(){
        ...
        if($this->params['paginUse'])
        {
            ...
            if($paginPrev>=1){
                $html.= '<a href="" onClick="AG_form_submit_'.$this->articleID.'('.$this->index.','.$paginPrev.',\''.$this->imagesFolderName.'\'); return false;" class="AG_pagin_prev prev">'.JText::_( "" ).'</a>';
            }
            ...
            if($paginNext<=ceil($this->paginImgTotal/$this->params['paginImagesPerGallery'])){
                $html.= '<a href="" onClick="AG_form_submit_'.$this->articleID.'('.$this->index.','.$paginNext.',\''.$this->imagesFolderName.'\'); return false;" class="AG_pagin_next next">'.JText::_( "" ).'</a>';
            }
            $html.= '<br style="clear:both"></div>';
                }
            }

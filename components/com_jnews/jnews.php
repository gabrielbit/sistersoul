...
$captchaWidth = JRequest::getVar('captchawidth');
$captchaHeight = JRequest::getVar('captchaheight');
...
if( version_compare(JVERSION,'1.6.0','<') ){ //j15
	require_once( $mainframe->getPath( 'front_html', JNEWS_OPTION  ) );
}else{ //j16
	require_once( JApplicationHelper::getPath('front_html', JNEWS_OPTION ) );
}
...
switch ($action) {
	...
	case ('mailing') :
		frontEnd::mailingOptions( $action, $task, $listId, $mailingId, $subscriberId, $lisType, $artId, $contentType, $blogId, $blogType, $dealId, $dealType, $tmplid, $flexiid, $flexitype, $captchaWidth, $captchaHeight);
		break;
	...
	case ('log') :
		jNews_ProcessMail::logStatistics($mailingId, $subscriberId);
		break;
	...
	case ('noredsubscribe'):
		...
		$modsub->rhtml = JRequest::getVar('modreceivehtml','');
...
//					$config['lasttime_cron_triggerred'] = jnews::getNow();

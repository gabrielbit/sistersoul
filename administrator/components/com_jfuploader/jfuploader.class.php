...
class JFUHelper {
 ...
 function getProfileId($sel_id, $id, $my) {
   ...
   if ($sel_id == 1) { // we have to find the right profile!
      ...
      $database->setQuery("SELECT j.id FROM #__joomla_flash_uploader j, #__joomla_flash_uploader_user u WHERE j.gid='" . $id . "' AND u.profile=j.id AND  u.user=" . $uid . " order by j.id");

...
//		jNews_Menu::_cPanelBtn();
	 ...
	 function PREVIEWMAILING($task) {
		...
		if ( $GLOBALS[JNEWS.'level'] > 1 && !jnews::isLocal() ) {
			...
			$linkSpam = jNews_Tools::completeLink( 'option='.JNEWS_OPTION.'&act=mailing&task=testspam&mailingid='.$mailingid.'&tmpl=component', true, false, true );
	 		if( version_compare(JVERSION,'1.6.0','<') ){
				$html = '<a class="modal" href="'. $linkSpam. '"> <img src="'. JNEWS_PATH_ADMIN_IMAGES .'toolbar'.DS.'block.png"> <br/><center>'._JNEWS_MENU_SEND_TESTSPAM.'</center> </a>';
	 		}else{ //j16
		    	$html = '<a class="modal" href="'. $linkSpam.'" rel="{handler: \'iframe\', size: {x: 1050, y: 800}}" > <img src="'. JNEWS_PATH_ADMIN_IMAGES .'toolbar'.DS.'block.png"> <br/><center>'._JNEWS_MENU_SEND_TESTSPAM.'</center> </a>';
	 		}
		...
		if($listype==1){
			$bar = & JToolBar::getInstance('toolbar');
			$linkViewOnline = jNews_Tools::completeLink( 'option='.JNEWS_OPTION.'&act=mailing&task=view&mailingid='.$mailingid.'&tmpl=component', true, false, true );
	 ...
	 function TEMPLATES_MENU(){
	 	if(class_exists('jNews_TemplatesHTML')){
	 		...
	 		if( version_compare(JVERSION,'1.6.0','<') ){ //j15
				$html = '<a class="modal" href="'. jNews_Tools::completeLink( 'option='.JNEWS_OPTION.'&act=templates&task=tempupload' , true, false, true ). '"> <img src="'. JNEWS_PATH_ADMIN_IMAGES .'toolbar'.DS.'upload.png"> <br/><center>'._JNEWS_FILES_UPLOAD.'</center> </a>';
	 		}else{ //j16
		    	$html = '<a class="modal" href="'. jNews_Tools::completeLink('option='.JNEWS_OPTION.'&act=templates&task=tempupload', true, false, true ).'" rel="{handler: \'iframe\', size: {x: 850, y: 500}}" > <img src="'. JNEWS_PATH_ADMIN_IMAGES .'toolbar'.DS.'upload.png"> <br/><center>'._JNEWS_FILES_UPLOAD.'</center> </a>';
	 		}

			$bar->appendButton( 'Custom', $html );
			JToolBarHelper::divider();
			$html = '<a target="_NEW" href="http://www.joobi.co/index.php?option=com_jlinks&controller=redirect&link=newsletter-templates"> <img src="'. JNEWS_PATH_ADMIN_IMAGES .'toolbar'.DS.'more_templates.png"> <br/><center>'._JNEWS_MORE_TEMPLATES.'</center> </a>';
			$bar->appendButton( 'Custom', $html );
			JToolBarHelper::divider();
	 ...
	 function NEWTEMPLATE(){
		...
		JToolBarHelper::divider();
	 	...
	 	if(class_exists('jNews_TemplatesHTML')){
			...
			$bar = & JToolBar::getInstance('toolbar');
			$HTML = '<a class="modal" title="'._JNEWS_MAILING_TAG.'" href="'. jNews_Tools::completeLink('option='.JNEWS_OPTION.'&act=tags', true, false, true ).'" rel="{handler: \'iframe\', size: {x: 850, y: 500}}"> <img src="'. JNEWS_PATH_ADMIN_IMAGES .'toolbar'.DS.'tag.png"/> <br/><center>'._JNEWS_MAILING_TAG.'</center> </a>';
			$bar->appendButton( 'Custom', $HTML );
	 		JToolBarHelper::divider();
	 		JToolBarHelper::customX('apply', 'applyT.png', 'applyT.png', _JNEWS_MENU_APPLY , false);
			JToolBarHelper::customX('save', 'saveT.png', 'saveT.png', _JNEWS_SAVE , false);
...
//			jNews_Menu::_cPanelBtn();
	  ...
	  function EDITTEMPLATE(){
	 	if(class_exists('jNews_TemplatesHTML')){
			...
			$html = '<a class="modal" onclick="getTemplate()" title="'._JNEWS_MENU_PREVIEW.'" href="'. jNews_Tools::completeLink( 'option='.JNEWS_OPTION.'&act=templates&task=preview&template_id='.$template_id , true, false, true ).'" rel="{handler: \'iframe\', size: {x: 850, y: 500}}"> <img src="'. JNEWS_PATH_ADMIN_IMAGES .'toolbar'.DS.'preview_template.png"> <br/><center>'._JNEWS_MENU_PREVIEW.'</center> </a>';
			$HTML = '<a class="modal" title="'._JNEWS_MAILING_TAG.'" href="'. jNews_Tools::completeLink('option='.JNEWS_OPTION.'&act=tags', true, false, true ).'" rel="{handler: \'iframe\', size: {x: 850, y: 500}}"> <img src="'. JNEWS_PATH_ADMIN_IMAGES .'toolbar'.DS.'tag.png"/> <br/><center>'._JNEWS_MAILING_TAG.'</center> </a>';
...
//		jNews_Menu::_wizardBtn();
	 ...
	 */
	  ...
	  */
	  function _backBtn(){
	  	$html = '<a href=\'javascript:history.go(-1)\'> <img src="'. JNEWS_PATH_ADMIN_IMAGES .'toolbar'.DS.'backT.png"/> <br/><center>'._JNEWS_MENU_BACK.'</center> </a>';
		$bar = & JToolBar::getInstance('toolbar');
		$bar->appendButton( 'Custom', $html );
		return true;
	  }
	  ...
	  */
	  function _cPanelBtn(){
	  	$html = '<a href=\'index.php?option='.JNEWS_OPTION.'\'> <img src="'. JNEWS_PATH_ADMIN_IMAGES .'toolbar'.DS.'cpanelT.png"/> <br/><center>'._JNEWS_MENU_CPANEL.'</center> </a>';
		$bar = & JToolBar::getInstance('toolbar');
		$bar->appendButton( 'Custom', $html );
		return true;
	  }
	  ...
	  */
	  function _editMailingPrev(){
		...
		$mailingid = ($mailingid==0) ? jNews_Mailing::getLastMailingId() : $mailingid;
	  	...
	  	$html = '<a href="index.php?option='.JNEWS_OPTION.'&act=mailing&task=edit&listype='.$mailingType.'&mailingid='.$mailingid.'&listid='.$listId.'"> <img src="'. JNEWS_PATH_ADMIN_IMAGES .'toolbar'.DS.'editT.png"/> <br/><center>'._JNEWS_MENU_EDIT.'</center> </a>';
		$bar = & JToolBar::getInstance('toolbar');
		$bar->appendButton( 'Custom', $html );
		return true;
	  }

	function _jnewsRefresh(){
		$html = '<a onclick ="javascript: submitbutton(\'refresh\')" href="#"> <img src="'. JNEWS_PATH_ADMIN_IMAGES .'toolbar'.DS.'refreshT.png"/> <br/><center>'._JNEWS_MENU_REFRESH_STATS.'</center> </a>';
		$bar = & JToolBar::getInstance('toolbar');
		$bar->appendButton( 'Custom', $html );
		return true;
	}

	function _jnewsGenerate(){
		$html = '<a onclick ="javascript: submitbutton(\'generate\')" href="#"> <img src="'. JNEWS_PATH_ADMIN_IMAGES .'toolbar'.DS.'generate.png"/> <br/><center>'._JNEWS_MENU_GENERATE_STATS.'</center> </a>';
		$bar = & JToolBar::getInstance('toolbar');
		$bar->appendButton( 'Custom', $html );
		return true;
	}

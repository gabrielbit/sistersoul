...
class jNews_TemplatesHTML {
	...
	function displayTemplateList($templates, $forms, $start, $limit, $templatesearch, $action, $setLimit=null, $setSort=null){
		...
		<table class="joobilist">
			<thead>
				<tr>
					...
					<th width="2%" class="title"><?php echo JHTML::_('grid.sort', _JNEWS_TEMPLATE_PUBLISH, 'template_id', $setSort->orderDir,$setSort->orderValue); ?>ID</th>
				</tr>
			</thead>
			</form>
			<?php
					...
					<td align="center">
						<span class="editlinktip">
							...
							if(empty($template->thumbnail)){
								...
								//echo jnews::imageResize($path , '100', '100',$template->name);
	...
	function previewTemplate($template , $forms){
...
//		$textarea .= sizeof($styles > 0) ? jNews_Templates::convertArrayStyles($styles) : _JNEWS_CUSTOMCSS;
	...
	/**
	 * <p>Function to create the view of the template list where the user can assign a template to a newsletter.</p>
	 * @param mixed $templates - the array object list of templates in the database

	 */
	 function assignTemplate($templates, $forms, $setLimit, $templatesearch){
	 	JHTML::_('behavior.modal');
	 	JHTML::_('behavior.mootools');
	 	
	 	$action = JRequest::getString('act', '', 'request');
	 	//$js = "function assignTemplate(templateid){var templatebody = window.document.getElementById('templatebody_'+templateid).innerHTML;	var templatealtbody = window.document.getElementById('templatealtbody_'+templateid).innerHTML; window.top.changeTemplate(templatebody, templatealtbody,templateid);	window.top.document.getElementById('sbox-window').close();}";
		...
		?>
	 	...
	 	<?php
			 ...
			 if($template->availability==1 && $template->published == 1){
				...
				$findposwww=strpos($template->thumbnail,'www.');
				if ( ($findposhttp === false ) || ($findposwww === false) ){
					//$path = JNEWS_PATH_ADMIN_THUMBNAIL_SHOW.$template->thumbnail;
	 	...
	 	?>
			<div class="template-container" onclick="assignTemplate(<?php echo $template->template_id;?>);">
				...
				<div id="templatealtbody_<?php echo $template->template_id;?>" style=""><?php echo $template->altbody; ?></div>
			</div>
		<?php
	 		}
	 	}

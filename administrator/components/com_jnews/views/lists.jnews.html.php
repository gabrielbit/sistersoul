...
### license GNU GPLv3 , link http://www.joobi.co
 ...
 class jNews_ListsHTML {
	 ...
	 function showListingLists($lists, $action, $task, $forms, $show, $listsearch='', $limit=0, $setLimit=null, $totalSubs=null, $setSort=null, $totalUnSubs=null ) {
...
//				$link = jNews_Tools::completeLink($link,$backendLink);
//				$link = jNews_Tools::completeLink($link,false);
				$link = JRoute::_($link);
			...
			if (($list->list_type == 1 or $list->list_type == 7/*changed from 2 to 7*/) && $GLOBALS[JNEWS.'show_archive']=='1' ) {
				...
				$linkArchive = 'option='.JNEWS_OPTION.'&amp;act=mailing&amp;listid=' .$list->id . '&amp;listype=' .$list->list_type .'&amp;task=archive' . $item;
//				$linkArchive = jNews_Tools::completeLink($linkArchive,$backendLink);
	...
	echo jnews::setLegend();
	 ...
	 function prepList($listEdit, $listType) {
		...
		$lists['list_templates'] = JHTML::_('select.genericlist', $list_templates,'template', 'class="inputbox"','value', 'text', $templateID );
	...
	return $lists;
	 ...
	 function editList($listEdit, $forms, $show, $listType=1) {
	...
	function description($listEdit, $lists, $show, $html, $listType) {
		...
		if($listType == 2){
	 	...
	 	<tr>
			...
			<?php
				...
				jNews_MailingsHTML::lists(null, $lists, $show, $listType, $listEdit->id, true);//added another parameter for the list type ---mary
			 ?>
			 </td></tr></table>
			</td>	 	</tr>
			<?php
...
<?php if ($show['sender_info']) {?>
	...
	function subscription($listEdit, $lists, $show, $html, $listType) {
		$mainframe = JFactory::getApplication();
		$editor =& JFactory::getEditor();
...
<?php }
	 ...
	 */
	function frontendManagement($listEdit, $lists, $show, $html) {
		$mainframe = JFactory::getApplication();
		?>
		<fieldset class="jnewscss">

...
function editSubscriberFE($subscriber, $listings, $queues, $forms, $access=false, $frontEnd=false, $cb=false ){
		...
		for($count = 1; $count <= $nRows; $count++){
			if (!$cb) {
				...
				if(empty($subscriber->email)) $subscriber->email='';
		...
		$html .= '<input id="aca_22" type="button" value="" alt="addmore" name="addmore" onclick="addmorefields();" /></div>';
		$html .= '</fieldset></div>';
		return $html;
	 ...
	 function editSubscriber($subscriber, $listings, $queues, $forms, $access=false, $frontEnd=false, $cb=false ) {
		...
		$html .= '</table>';
		$html .= '</fieldset></div>';
		//$html .=  jNews_SubscribersHTML::showSubscriberLists($subscriber, $listings, $queues, $frontEnd, $access);//--original
	 ...
	 function showSubscribers($subscribers, $action, $listId, &$lists, $start, $limit, $total, $showAdmin, $theLetterId, $emailsearch, $forms, $setLimit=null, $front = false, $setSort=null) {
...
//						$link = jNews_Tools::completeLink($link,false);
	 ...
	 function export($action, $listId) {
		?>
		<form action="index.php?option=<?php echo JNEWS_OPTION; ?>&act=<?php echo $action; ?>&listid=<?php echo $listId; ?>" method="post" name="adminForm" >
			<input type="hidden" name="task" value="" />
			<br /><br />
			<?php echo _JNEWS_EXPORT_TEXT.' #: '.$listId; ?><br /><br />
			<input type="button" value="Export" class="button" onclick="submitbutton('doExport')" />
		</form>
		<?php
	 }

	 function import($action, $lists,$listId) {
		...
		<table border="0" width="100%" cellpadding="1" cellspacing="1" class="joobilist">
			<tbody>
		<?php

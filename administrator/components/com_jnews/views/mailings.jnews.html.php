...
class jNews_MailingsHTML {
	function previewMailingHTML($mailingId, $listId, $listType, $html){
	?>
...
/**
	 * This function is used for the view of sendReady for manual sending
	 */
	 function sendReady($mailingId, $listId, $listType){
		?>
	 ...
	 function showMailingList($mailings, &$lists, $start, $limit, $total, $emailsearch, $listId, $listType, $forms, $show, $action, $setLimit=null, $setSort=null) {

		$Itemid= JRequest::getVar('Itemid');
		$mainframe = JFactory::getApplication();

		$my	=& JFactory::getUser();
		$mainPath = JNEWS_PATH_ADMIN_IMAGES.'header/';
	...
	<table class="joobilist">
		<thead>
			<tr>
				...
				if(empty($listType) OR $listType ==0){
	...
	<tr class="row<?php echo ($i + 1) % 2;?>">
		...
		$link = jNews_Tools::completeLink( $link, true, true, false );	// $backendLink	// JNEWS_JPATH_LIVE .'/'. 
		...
		if($mainframe->isAdmin()){
			...
			echo '</a>';
			 ...
			 if ( $GLOBALS[JNEWS.'level'] > 1  OR  $mailing->published == 1){
			 	echo '<a  href="'.$link.'" >';
				echo $mailing->subject;
				echo '</a>';
			 }else{
		...
		<td align="center"><center>
			...
			$listtypemailing = JRequest::getVar('listype');
...
<script language="javascript" type="text/javascript">
	...
	echo jnews::setLegend();
	 ...
	 function editMailing($mailingEdit, $new, $listId, $forms, $show, $isEdit=false) {
		$lists = array();
		$mainframe =& JFactory::getApplication();
	...
	function lists($mailingEdit, $list, $show, $lType, $arId, $indicate=false, $isEdit=false) {
...
?>
		...
		<table class="jnewstabletable" cellspacing="1" border="0">
			<tbody>
				<tr>
					...
					<?php if (JRequest::getVar('act')=='mailing' AND JRequest::getVar('listype')==2 /*$show['delay']*/) { ?>
							<tr>
								<td class="key">
									<span class="editlinktip">
									<?php
					...
					<?php } ?>
					<?php if (JRequest::getVar('act')=='mailing' AND JRequest::getVar('listype')==1 /*$show['pub_date'] AND $GLOBALS[JNEWS.'listype2']=='1'*/ AND class_exists('jNews_Auto')) { ?>
						<tr>
							<td class="key">
								<span class="editlinktip">
								<?php
	...
	function layoutFE($mailingEdit, $lists, $show, $isEdit=false) {
 ...
 if ($show['htmlcontent']) {
		 	...
		 	echo '<tr><td colspan="2">'; //added by eve
...
<?php
	...
	<?php }else{
		...
		$config_tabs = new MosTabsjNews(0);
		 	$config_tabs->startPane('acaMailingOptions');
		 	 if ($show['htmlcontent']) {
		 	$config_tabs->startTab(_JNEWS_HTML_VERSION, 'acaMailingOptions.general');
			echo $editor->display( 'jnewsContent',  $mailingEdit->htmlcontent, '100%', '1000', '80', '30' ) ;
			$config_tabs->endTab();
		 	 }
			if (($show['textcontent'])) {
			$config_tabs->startTab(_JNEWS_NONHTML_VERSION, 'acaMailingOptions.general');?>
	...
	function layoutBE($mailingEdit, $lists, $show, $isEdit=false) {
...
//				$urlInstances = JURI::getInstance()->toString();
				$urlInstances = JURI::getInstance();
...
//				$url = $urlInstances->_uri;
				...
				//we should save the mailing firts
...
<?php
	...
	/**
	 * <p>Function to create the header or the subject area of the smart newsletter</p>

	 */
 	function smartNewsHead($mailingEdit, $lists, $show){
		$option = array();
			$my =& JFactory::getUser();
			$acl =& JFactory::getACL();
		...
		if(!empty($mailingEdit->cat_id))JRequest::setVar('cat_id', base64_encode($mailingEdit->cat_id));

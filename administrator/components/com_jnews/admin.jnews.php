...
$action	= JRequest::getVar( 'act', '', '', 'string', JREQUEST_ALLOWRAW );
...
function configuration($action, $task ) {
	...
	switch ($task) {
		 ...
		 case ('syncUsers') :
			...
			backHTML::_header( _JNEWS_MENU_CONF , 'configuration.png' , $message , $task, $action );
			jNews_ConfigHTML::showConfigEdit($GLOBALS);
		 	break;
		 ...
		 case ('sendtest'):
			...
			### create content
			$mail->IsHTML(true);
			$mail->Body = '<p>This message has been sent at '. date('l, j F Y h:i:s', jnews::getNow( 0, true ) ).' from '.JNEWS_JPATH_LIVE.' to test your mail configuration.</p><br/>' .
					'<p style="color:green;">'._JNEWS_SENDTEST_CONFIGSUCC.'</p>';
...
//						joobiCron('No');
//
//						//we enable jNews Cron System Plugin
//						if(version_compare(JVERSION,'1.6.0','<')){ //j15
//							$db->setQuery("UPDATE `#__plugins` SET `published` = 1 WHERE `element`='jnewscron' ");
//						}else{ //j16
//							$db->setQuery("UPDATE `#__extensions` SET `enabled` = 1 WHERE `type` = 'plugin' AND `element`='jnewscron' ");
//						}
//						$db->query();
//						break;
//
//					case('2'): //Joobi Cron
//						//we update the Joobi Cron Record
//					 	joobiCron('Yes');
//
//					 	//and we disable the jNews Cron System Plugin
//						if(version_compare(JVERSION,'1.6.0','<')){ //j15
//							$db->setQuery("UPDATE `#__plugins` SET `published` = 0 WHERE `element`='jnewscron' ");
//						}else{ //j16
//							$db->setQuery("UPDATE `#__extensions` SET `enabled` = 0 WHERE `type` = 'plugin' AND `element`='jnewscron' ");
//						}
//						$db->query();
//						break;
//
//					case('3'): //External Cron
//						//we disable both Joobi Cron
//						joobiCron('No');
//
//						//and jNews Cron System Plugin
//						if(version_compare(JVERSION,'1.6.0','<')){ //j15
//							$db->setQuery("UPDATE `#__plugins` SET `published` = 0 WHERE `element`='jnewscron' ");
//						}else{ //j16
//							$db->setQuery("UPDATE `#__extensions` SET `enabled` = 0 WHERE `type` = 'plugin' AND `element`='jnewscron' ");

	...
	function _header($title, $img , $message, $task, $action ) {
	 ...
	 function _header15($title, $img , $message, $task, $action ) {
	  ...
	  $document=& JFactory::getDocument();
       if($mainframe->isAdmin()){
          $document->addStyleSheet( 'components/'.JNEWS_OPTION.'/cssadmin/jnews.css' );
       } else {
           $document->addStyleSheet('administrator/components/'.JNEWS_OPTION.'/cssadmin/jnews.css' );
       }		
	...
	<?php
	 ...
	 function controlPanel() {
    	...
    	$doc =& JFactory::getDocument();
       	$doc->addStyleSheet('components/'.JNEWS_OPTION.'/cssadmin/jnews.css');
...
<?php
	...
	function installPRO($button='',$return=''){
		...
		$proStore .= '<a target="_blank" href="http://www.joobi.co/r.php?l=jNewsPro">';
	...
	function installPlus($button='',$return=''){
		...
		$html .= '</div>';
		$html .= '<div style="float:left; padding-left: 30px; width:470px;">';
		$html .= '<fieldset class="jnewscss" style="padding: 10px; text-align: left; -moz-border-radius:3px;-webkit-border-radius:3px;">';
		$html .= '<legend><strong>'._JNEWS_NOTIF_UPDATE.'</strong></legend>';
		$listID = 11;
		$name = 'jNews Updates';
		$module = '<div class="module_green" style="width: 200px;"><div><div><div><div class="jnews_module">';
		$module .= '<!--  Begining : jNews Form  -->';
		$module .= '<form action="http://www.joobi.co/index.php?option='.JNEWS_OPTION.'" method="post" name="modjnewsForm">';
		$module .= '<input id="wz_31" type="checkbox" class="inputbox" value="1" name="subscribed[1]" checked="checked" />';
		$module .= '<input type="hidden" name="sub_list_id[1]" value="'.$listID.'" /> '._JNEWS_INSTALL_UPDATES;
		$module .= '<input id="wz_11" type="text" size="30" value="" class="inputbox" name="name" onblur="if(this.value==\'\') this.value=\'Name\';" onfocus="if(this.value==\'Name\') this.value=\'\' ; " />';
		$module .= '<br><input id="wz_12" type="text" size="30" value=" class="inputbox" name="email" onblur="if(this.value==\'\') this.value=\'E-mail\';" onfocus="if(this.value==\'E-mail\') this.value=\'\' ; " />';
		$module .= '<br><input id="wz_2" type="checkbox" class="inputbox" value="1" name="receive_html"  checked="checked"  />';
		$module .= _JNEWS_RECEIVE_HTML;
		...
		$plusStore .= '<a target="_blank" href="http://www.joobi.co/r.php?l=jNewsPLus">';

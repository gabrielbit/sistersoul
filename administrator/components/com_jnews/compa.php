...
// params $back force the BE or if we dotn want to specify the base URL
	function completeLink( $link, $back=true, $sef=true, $popup=false, $addCompOption=true ) {
//		static $JoomVersion = null;
		if($link=="#") return;	// $mainframe->isAdmin()
//		if ( !isset($JoomVersion) ) $JoomVersion = version_compare(JVERSION,'1.6.0','<') ? true : false;
		if($back){
			if ( $popup ) {
	...
	function showIcon($image,$text,$text2 = '',$option = 1) {
		...
		if ( $image == 'query.png' || $image == 'systeminfo.png' || $image == 'month_f2.png')
			...
			$path = JNEWS_PATH_ADMIN_IMAGES . $image;

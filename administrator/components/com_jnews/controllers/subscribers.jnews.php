...
function subscribers( $action, $task, $userid, $listId, $cid, $front=false ) {
	...
	$css = '.icon-48-subscribers { background-image:url('.JNEWS_PATH_ADMIN_IMAGES .'header/subscribers.png)}';
	...
	$subscriber=null;
	$subscriber->user_id = intval(JRequest::getVar('user_id', 0));
	$subscriber->name = JRequest::getVar('name', '');
	$subscriber->email = JRequest::getVar('email', '');
	$subscriber->receive_html = intval(JRequest::getVar('receive_html', 0));
	...
	if(empty($subscriberId)){//if it is a new user the confirmed depends if the require confirmation is turned on
		if ($GLOBALS[JNEWS.'require_confirmation'] == '1') {
			...
			$subscriber->confirmed = intval(JRequest::getVar('confirmed', 0));
		}
	}else{
		$subscriber->confirmed = intval(JRequest::getVar('confirmed', 0));
	}
	...
	switch ($task) {
		...
		case ('doImport') :
		    ...
		    backHTML::_header( _JNEWS_MENU_SUBSCRIBERS , $img , $message, $task, $action );
		    $message = ( !empty($message) ) ? $message : ( $message===false ? 'Import failed' : _JNEWS_IMPORT_FINISHED );
			if($mainframe->isAdmin()){
...
//			$subs = jNews_Subscribers::getSubscribers($setLimit->start, $setLimit->end, $emailsearch, $setLimit->total, $listId, '','','','sub_dateD','', $ownedlists, $setSort, $subscirberTypeID );//added one parameter for mailid
//			$setLimit->total = ( !empty($subs) ) ? count( $subs ) : $setLimit->total;
//		}

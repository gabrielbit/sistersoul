...
function templates( $action, $task, $template_id) {
	$my	=& JFactory::getUser();
	$css = '.icon-48-templates { background-image:url('.JNEWS_PATH_ADMIN_IMAGES .'header/templates.png)}';
	...
	switch ($task) {
		...
		case ('publish') :
			if(!$checkToggle) JRequest::checkToken() or die( 'Invalid Token' );
	     	$message = jnews::printYN( jNews_Templates::updateTemplate($template_id, 'published',true) ,  'Successfully published template!' , 'Error publishing the template!' );
			jNews_Tools::redirect( 'index.php?option='.JNEWS_OPTION.'&act=templates', $message);
			break;

	   	case ('unpublish') :
	   		if(!$checkToggle) JRequest::checkToken() or die( 'Invalid Token' );
	   		$condition = jNews_Templates::updateTemplate($template_id,'published', false);
...
//	   		$message = jnews::defaultYN( jNews_Templates::updateTemplate($template_id,'published', false) ,  'Successfully unpublished template!' , 'Unable to unpublished default  template!' );
		...
		case ('delete') :
			...
			if (!$isDefault) {
				$message = jnews::printYN( jNews_Templates::deleteTemplate($template_id) ,  _JNEWS_TEMPLATE. _JNEWS_SUCCESS_DELETED , _JNEWS_ERROR );
			}else{

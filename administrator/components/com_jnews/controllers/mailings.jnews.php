...
//			$queueC->updateSenddateToNow($mailingId);
		...
		case ('testspam') :
			...
			$link = 'http://www.joobi.co/r.php?l=SpamCheck&alt=jnewsdoc_glossary';
		...
		case ('preview') :
			...
			$css = '.icon-48-preview{ background-image:url('.JNEWS_PATH_ADMIN_IMAGES .'header/preview.png)}';
		...
		case ('show') :
			...
			$mySess =& JFactory::getSession();
			//$mailingType = $mySess->get('listype', '', 'LType');
			$mailingType=JRequest::getVar('listype', 0);//---mary
			$listId=JRequest::getVar('listid', 0);//---mary

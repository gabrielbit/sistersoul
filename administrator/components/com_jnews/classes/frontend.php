 ...
 class frontEnd {
	...
	function showLists($subscriberId, $listId,$lisType,$action, $task ) {
		...
		if(!empty($my->id)){
			...
			if( (empty($listsAddEdit) && empty($ownedlists) && !jnews::checkPermissions('admin')) && !jnews::checkPermissions($gid) ){ //traces
				frontHTML::showPanel();
				return true;
			}
		}
...
//					$forms['main'] = "<form action='$linkForm' method='post' name='adminForm' enctype='multipart/form-data' onsubmit='submitbutton();return false;'> \n " ;
					...
					// menus for list edit
					// menu save
//					$linkForm = 'option='.JNEWS_OPTION.'&act=list&listype='.$lisType.'&siteend=1&Itemid='.$Itemid;
...
//					$linkForm = 'option='.JNEWS_OPTION.'&act=list&listype=1&siteend=1&Itemid='.$Itemid;
//					$linkForm = jNews_Tools::completeLink( $linkForm, false );
					$linkForm = '#';
					$menuCancel->link = $linkForm;
...
//						jNews_Tools::redirect('index.php?option='.JNEWS_OPTION.'&act=list&listype='.$lisType.'&siteend=1');
			...
			default:
				...
				$link = 'option='.JNEWS_OPTION.'&act='.$action;
				...
				if (jnews::checkPermissions('admin') || jnews::checkPermissions($gid) || !empty($listsAddEdit)) {//traces
					if ($mainframe->isAdmin()){
				...
				if ( !empty($lists) || jnews::checkPermissions($gid)  ) { //traces
					$menuA = null;
					if( $my->id > 0 ){
						$link = 'option='.JNEWS_OPTION;
						$link = jNews_Tools::completeLink($link,false);
						...
						if( ( $GLOBALS[JNEWS.'enable_jsub'] && (!empty($ownedlists) ) || jnews::checkPermissions('admin') ) || jnews::checkPermissions($gid)){//traces
							...
							if( $GLOBALS[JNEWS.'enable_jsub'] && (!empty($ownedlists) ) ){
								...
								$menuNew->onclick->js = 'javascript:history.go(-1)';
								$menuNew->title = 'New';
...
//							$menuA['back'] = $menuBack;
					...
					if ( class_exists('jNews_Pro') ) {
						...
						foreach( $lists as $list ) {
							$bit = jnews::checkPermissions($list->acc_level); //come back here eve
							if ( $bit ) {
								$access = true;
								break;
							}
					...
					frontHTML::formEnd('', $go);
 					...
 					//echo jnews::printYN( '', '', 'No Lists available for this type of list', '' );
	...
	function mailingOptions( $action, $task, $listId, $mailingId, $subscriberId, $listType, $artId, $contentType, $blogId, $blogType, $dealId, $dealType, $tmplid, $flexiid, $flexitype, $captchaWidth = 80, $captchaHeight = 25 ) {
...
//				$newpath= JNEWS_JPATH_LIVE_NO_HTTPS.'/components/'.JNEWS_OPTION.'/captcha/CaptchaSecurityImages.php?width='.$GLOBALS[JNEWS.'captcha_width'].'&height='.$GLOBALS[JNEWS.'captcha_height'].'&characters=5&esc='. $newesc.'&encpwd='.crypt($GLOBALS[JNEWS.'captcha_code'],$GLOBALS[JNEWS.'captcha_code']);
				$newpath= JNEWS_JPATH_LIVE_NO_HTTPS.'/index.php?option='.JNEWS_OPTION.'&act=captcha&tmpl=component&width='.$captchaWidth.'&height='.$captchaHeight.'&characters=5&esc='. $newesc.'&encpwd='.crypt($GLOBALS[JNEWS.'captcha_code'],$GLOBALS[JNEWS.'captcha_code']);
			...
			case 'edit':
				...
				if($GLOBALS[JNEWS.'enable_jsub']){
					if(!empty($my->id))$ownedlists = jNews_Lists::getOwnedlists($my->id);
					if(!empty($ownedlists) OR jnews::checkPermissions('admin') ){
						if( empty($mailingId) ){
					...
					//traces
					if(jnews::checkPermissions('admin', 0, 18) || jnews::checkPermissions($gid) || !empty($listsAddEdit ) ){
						if( empty($mailingId) ){
							echo '<script> javascript:alert("'. _JNEWS_SELECT_MAILING .'"); javascript:history.go(-1); </script>';
						} else {
							frontEnd::mailingEdit($subscriberId, $mailingId, $listId, $listType, 'mailing');//savemailing
...
//					$linkZ = 'option='.JNEWS_OPTION;
//					$linkZ = jNews_Tools::completeLink($linkZ,true);
//					$forms['main'] = '<form method="post" action="index.php?option='.JNEWS_OPTION.'" onsubmit="submitbutton();return false;" name="mosForm" >'."\n\r";
					$mainLink = JRoute::_( 'index.php?option='.JNEWS_OPTION );
				...
				if( jnews::checkPermissions('admin') || jnews::checkPermissions($gid) || !empty($listsAddEdit) ) {
						...
						if($listType==1 && $GLOBALS[JNEWS.'level'] > 2){
							if( !empty($acc_level) && jnews::checkPermissions($acc_level )) $menuA['sendready'] = $menuSend;
						}
			...
			case ('send') :
				...
				$queueCount = $queueC->getQueueCount( $mailingId );
			...
			default:
				//frontHTML::FEmenu();
				/*
				if (jnews::checkPermissions($gid) || jnews::checkPermissions('admin') || !empty($listsAddEdit))*/ 
				$viewarchive = false;
				if(empty($my->id)) $viewarchive = true;
				frontEnd::showMailingsFront( $task, $action , $subscriberId, $listId, $listType, $viewarchive, _JNEWS_MENU_MAILING); //$viewarchive=false;
				break;
		}
  ...
  function checkMailingSave( &$mailingType ) {
	...
	function newSubscriber( $name, $email, $confirm=false, $modsub=null ) {
		$database =& JFactory::getDBO();
		$acl =& JFactory::getACL();
		...
		if($p==$GLOBALS[JNEWS.'url_pass'] and empty($fromSubscribe) and empty($fromFrontend)){
			$fromURL=true;
		}else{
			$fromURL=false;
		}
		...
		if( $canInsert ) {
			...
			if(!$modsub->red_subscription ){
				if($modsub != null){
					...
					$newSubscriber->receive_html =  $modsub->rhtml;
			...
			if ($GLOBALS[JNEWS.'require_confirmation'] and (!$confirm)) $newSubscriber->confirmed =  0;
			...
			if ( !jNews_Subscribers::saveSubscriber( $newSubscriber, $newSubscriber->id, true, $modsub->subscribedlists, $modsub->red_subscription) ) {
			...
			if ($needConfirm) {
				if(!$modsub->red_subscription ){
					...
					else $message = '<div id=mod_message><span style="color:blue;">'. _JNEWS_COMFIRM_SUBSCRIPTION. '</span></div>';
				}else{
					$message = jnews::printM('blue' , _JNEWS_COMFIRM_SUBSCRIPTION);
				}
			} else {
				if( !$modsub->red_subscription ){
					if ( $modsub->module_message || $modsub->effect == 2 ) $message ='<div id=mod_message>'. _JNEWS_SUCCESS_ADD_LIST. '</div>';
					else $message = '<div id=mod_message><span style="color:green;">'. _JNEWS_SUCCESS_ADD_LIST. '</span></div>';
				}else{
					$message = jnews::printM('green' , _JNEWS_SUCCESS_ADD_LIST);
				}
			}
			...
			if($GLOBALS[JNEWS.'level'] > 2){
				...
				if ( !empty($SubscribedList) ) {
					...
					foreach( $SubscribedList as $list ) {
						...
						if( ($list->subnotifysend==1) && !empty($list->subnotifymsg) ) {
							 $owner = null;
							 $owner->name = $list->sendername;
							 $owner->email = $list->senderemail;
							 jNews_ProcessMail::sendNotification( $list->subnotifymsg, $newSubscriber, $owner, $list, _JNEWS_SUBS_NOTIFYSUBJECT );
						}
	 ...
	 function subscriptions($subscriber, $subscriberId, $listId, $action) {
...
//			$menuA['cpanel'] = $menuCpanel;
   ...
   return true;
	 ...
	 //next to check
	 function changeSubscriptions($subscriber, $subscriberId, $cle='', $action) {
		$Itemid= JRequest::getVar('Itemid');
		$db=&JFactory::getDBO();
	...
	function remove($subscriber, $subscriberId, $cle='') {
		...
		if (!empty($subscriberId) AND !empty($cle)) {
			...
			if ( md5($subscriber->email) == $cle ) {
				...
				foreach ($listsO as $key=>$list ) {
					...
					if (  ($GLOBALS[JNEWS.'level'] > 2) && ($list->unsubscribenotifyadmin == 1) && !empty($list->notifyadminmsg) ) {
						 $owner = null;
						 $owner->name = $list->sendername;
						 $owner->email = $list->senderemail;

						jNews_ProcessMail::sendNotification($list->notifyadminmsg, $subscriber, $owner, $list, _JNEWS_UNSUBS_NOTIFYSUBJECT);
					}
   ...
   function unsubscribeall($subscriber, $subscriberId, $cle=''){
   		$my =  &JFactory::getUser();
   	
		if ((!empty($subscriberId) AND !empty($cle)) OR !empty($my->id)) {
			   if ( md5($subscriber->email) == $cle OR !empty($my->id) ) {
	...
	function unsubscribe($subscriber, $subscriberId, $cle='', $mailingId, $action) {
		$Itemid= JRequest::getVar('Itemid');
		$db=&JFactory::getDBO();
...
//					}
	 ...
	 function showMailingsFront( $task, $action, $subscriberId, $listId, $listType='', $viewArchive, $pageTile) {
	   ...
	   //we check if the user has access to addedit mailing to a list
		...
		if(empty($listsAddEdit) && !empty($my->id) && !jnews::checkPermissions('admin')){
//					$message = jnews::printM( 'warning' , _NOT_AUTH );
//					break;
...
//		$linkMain = jNews_Tools::completeLink($linkMain,true);
		$mainLink = JRoute::_( 'index.php?option='.JNEWS_OPTION );
		...
		$db =& JFactory::getDBO();
		$query= 'SELECT * FROM `#__jnews_lists` WHERE `hidden` = 1 AND `published` = 1 AND list_type = '.$mailingType;
		$db->setQuery( $query );
		$lists = $db->loadObjectList();
...
//					$menuNew->link = $link;
					...
					$menuUnpub->link = '#';
					$menuUnpub->action = 'unpublished';
					$menuUnpub->onclick->custom = false;
...
//					$listId = $ownedlists[0];
	...
	function assignTag($action, $task){
		$doc =& JFactory::getDocument();
        $doc->addStyleSheet( JNEWS_JPATH_LIVE . '/administrator/components/'.JNEWS_OPTION.'/cssadmin/jnews.css');
      	$mailing = null;
	...
	function showSubscribersFE($listId = 0, $action, $task, $front = false){
		$my = &JFactory::getUser();
...
//		$menuA['back'] = $menuBack;
	...
	function statisticsFE($action, $task, $listId, $listType='', $mailingId, $message, $Itemid){
			$my = &JFactory::getUser();
			if(empty($my->id)) return true;

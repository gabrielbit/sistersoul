...
if(!$mainframe->isAdmin() AND empty($Itemid)){
 ...
 class jnews {
	...
	function displayInfo($messages,$type = 'success'){
	 ...
	 */
	function M($type , $message,$showMsgBck=true) {
	 ...
	 function messageMgmt($action, $task, $message) {
		...
		if (empty($message)) {
			...
			if ($GLOBALS[JNEWS.'mod_pub']==0) {
				...
				if ($total['mod_pub']==0 AND $GLOBALS[JNEWS.'act_totalmailing0'] < 3)
...
//			}
	...
	function checkPermissions($allowedGroups, $groups=null){
...
//	function checkPermissions($grouptoAccess){
//
//		$mainframe = JFactory::getApplication();
//		if($mainframe->isAdmin() ) return true;
//		$my = JFactory::getUser();
...
//		$allowedGroups = explode(',',$gid);
//
//		foreach($usergroups as $usergroup){
//			$allgroups[]=$usergroup->value;
//		}
//
//		if( ($grouptoAccess == 'all') || ($grouptoAccess == 'admin') || ($grouptoAccess == 'none') || ($grouptoAccess == 'registered') || ($grouptoAccess == 'Registered' ) ) {
//
//			if($grouptoAccess == 'all' && $my->id > 0 ) $canAccess = $allgroups;
//			if ($grouptoAccess == 'admin' && $my->id > 0 ) $canAccess = array(24,25,7,8);
//			if($grouptoAccess == 'none') $canAccess[] = 0;
//
//			if(($grouptoAccess == 'registered' && $my->id > 0 ) || ($grouptoAccess == 'Registered' && $my->id > 0 ) ) {
//				$canAccess = array(18,2,$my->gid);
//			}
//
//		}else{
//			$canAccess = explode(',',$grouptoAccess);
//		}//emdif
//
//		if(is_array($canAccess)){
//			$inter = array_intersect($canAccess,$allowedGroups);
//
//			if(empty($inter)) {
//				 return false;
//			}else{
...
//
//		}else{
//			return in_array($canAccess,$allowedGroups);
//		}
//
//
//	}
...
/**
     ...
     */
	function getNow( $delay=0, $offset=false, $specificTime=null ) {
...
/**
   ...
   * @return $timeOffset;
   */
  	function calculateOffset($configOffset = ''){
...
//				$list->subscribemessage = '[CONFIRM]';
	 ...
	 * @return string $link
	*/
	function createToggleLink($act, $column, $variableName, $variableValue, $task='toggle', $listid = 0, $listype = 1){
		$link = null;
	...
	*/
	 ...
	 * @return $display
	*/
	function _pagination($setLimit, $onlyBox = false, $showTotal = false){
	 ...
	 * @return $html
	*/
	function setTop( $search=null, $message=null, $pagination=null, $filter=null, $onlyBox=false, $total=false ){
		$html = '';
	 ...
	 */
	 function setPaginationBot($pagination=null, $css=''){
//echo '0000000000';
	 	$html = '';
	...
	*/
	 ...
	 */
	function setLimitPagination($limittotal){
	 ...
	 */
	function setPageInfo($limittotal=null){
		$paginationStart = JRequest::getVar( 'pg' );
		$pageInfo = null;
		if( !empty($paginationStart) ){
			$limitstart = 0;
	 ...
	 */
	function EditorSetContent($editor, $name, $html){
	 ...
	 */
	function createDirectory($dir,$report = true){
	 ...
	 */
	function displayAccessRoles($map,$values){
	?>

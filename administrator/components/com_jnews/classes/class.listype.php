 ...
 class jNews_ListType {
   ...
   return $results;
	 ...
	 function getListsDropList($listId, $listType, $order) {
		$lists = array();
		$lists[0]->list_name = _JNEWS_SEL_ALL_SUB;
		$lists[0]->id = 0;
	 ...
	 function showType( $listType , $screen ) {
	 	...
	 	switch ($screen) {
			case 'editmailing':
				$className = JNEWS . $GLOBALS[JNEWS.'classes'.$listType];
			...
			case 'showListsFront':
				...
				$db =& JFactory::getDBO();
				$query= 'SELECT * FROM `#__jnews_lists` WHERE `hidden` = 1 AND `published` = 1 AND `list_type` = '.$listType;
				$db->setQuery( $query );

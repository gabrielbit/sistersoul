...
class jNews_Subscribers {
	...
	function getSubscribers( $start=-1, $limit=-1, $emailsearch, &$total, $listId, $mailingId, $blackList=0 , $confirmed=0, $order='', $mailId='', $ownedlists = 0, $setSort=null, $subType=1, $countUser=false ) {//added one parameter mailid
		$db=&JFactory::getDBO();
		$my	=& JFactory::getUser();
		$flag = false;
		$limitFlag = false;
		...
		if (!empty($emailsearch)) {
			if ($flag){
				if( is_numeric($emailsearch) ) $query .= ' AND M.id='. $emailsearch;
				else $query .= ' AND (M.email LIKE \'%' . $emailsearch . '%\' OR M.name LIKE \'%' . $emailsearch . '%\') ';
			}else{
				if( is_numeric($emailsearch) ) {
					$query .= ' WHERE M.id='. $emailsearch;
				}else {
					if($mainframe->isAdmin()){
						$query .= ' WHERE M.email LIKE \'%' . $emailsearch . '%\' OR M.name LIKE \'%' . $emailsearch . '%\' ';
					}else{
						if($ownedlists != 0 OR !empty($ownedlists) AND $listId == 0)
	...
	function getAdmins() {
	 ...
	 /**
	  * This function will save the subscriber information every new subscription or update subscription
	  ...
	  */
	 function saveSubscriber( $subscriber, &$subscriberId, $newSubscriber=false, $listids=null, $redirect=false ) {
...
//added condition  empty($subscriber->user_id) so that it will not send confirmation email during registration
			if ( ($GLOBALS[JNEWS.'require_confirmation'] == '1') && empty($subscriber->user_id) ){
				//added $subscriber
	...
	/**
	 * This function is used for CB Integration and FE Subscription
	 */
	function updateCBFESubscriber($userId=0, $user=null, $confirmed='0' ) {
		$oneSubscriber = null;
	...
	/**
	 * This function is used by jnews_cb plugin
	 */
	 function updateCBSubscribers( $force=false, $install=false ) {
		$db=&JFactory::getDBO();
	...
	function export($listId) {
			...
			$subscribers = jNews_Subscribers::getSubscribers(-1,  -1, '', $total, $listId, '', 1 , 2, 'name','');//added one parameter for mailid
	...
	/**
	 * This function is used to get the IP of the user
	 */
	function getIP(){
		$ip = '';
	...
	/**
	 * This function is used to get the users of the site who has access to the backend to specify the sender of the mailings
	 * @params $orderBy
	 * @params $limit
	 * @return $usersA as object
	 */
	function getUsers( $orderBy, $limit=100, $id=0 ){
 ...
 */
	function getSubscribersCount( $listId=0, $includeUnsub=true, $includeConf=false, $includeBlack=false ) {
		static $db=null;
		if( !isset( $db ) ) $db=&JFactory::getDBO();

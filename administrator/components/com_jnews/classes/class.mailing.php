...
class jNews_Mailing {
	 ...
	 function getMailings($listId, $listType, $start = -1, $limit = -1, $emailsearch='', $order='', $showOnlyPublished=true, $viewArchive=false, $setSort=null) {
		$my	=& JFactory::getUser();
		$db=&JFactory::getDBO();
	 ...
	 function getMailingInfoz($mailingId, $column='mailing_type' ) {
		static $result = array();
	 ...
	 function getOneMailingSmart( $mailingID, $issue_nb ) {
		$db=&JFactory::getDBO();
	 ...
	 function getOneMailing($list, $mailingId, $issue_nb, &$new, $send = false, $isEdit=false ) {
		$my	=& JFactory::getUser();
		$db=&JFactory::getDBO();
	 ...
	 * @return int $counts
	*/
	function countMailings($listId, $listType=0){
		$db=&JFactory::getDBO();
...
//			$senddate = jnews::getNow( $delay );
			$senddate = time() + ($delay*60);
		} else {
			if ( $senddate > time() ) {	// jnews::getNow()
				$published = 2;
			}
		}
...
//			if (substr_count($content, '[UNSUBSCRIBE]')<1) $content .= "<br/> [UNSUBSCRIBE] <br/>";
//			if (strlen($alt_content) > 10 AND substr_count($alt_content, '[UNSUBSCRIBE]')<1) $alt_content .= "\r\n [UNSUBSCRIBE]\r\n";
	 ...
	 function sendTestEmail($mailingId, $listId ){
	 ...
	 function insertMailingData($mailing) {
		...
		$xf = new jNews_Config();
//		$mailing->html .= '<!--jtag:jomsocial name=invisible-->';
		$query = "INSERT INTO `#__jnews_mailings` ( `mailing_type`,`template_id`,`send_date`, `subject`, `htmlcontent`, `textonly`,".
...
/* Function that will save mailing id and list id to its cross table jnews_listmailings
	 * @param int $mailingId - mailing id
	 * @param int $listId - list id
	*/
	function saveMailingList( $mailingId, $uPdatedlistIdA, $send_date, $mailing_type, $published, $newInsert=false, $queueSendDateUpdate=false, $queuePublishUpdate=false, $queueDelayUpdate=0 ) {
		$db=&JFactory::getDBO();
	 ...
	 function getSendDate($mailingId) {
    	...
    	/* Function that will remove the mailing and list entry from its cross table jnews_listmailings
    	 * @param int $mailingId - mailing id
    	 * @param int $listId - list id
    	*/
	...
	/* Function that will check if the checked entry exist in cross table jnews_mailings
	 * @param int $mailingId - mailing id
    	 * @param int $listId - list id
	...
	*/
	...
	/**
	 * This function is used to get the names of the lists where the mailing is sent to
	 */
	function getMailingListName( $listIds ) {
	...
	/* Function that will get all the list ids for the passed mailing id from its cross table jnews_mailings
	 * @param int $mailingId - mailing id
...
/** Function that will get all the mailing list ids for the passed list id from its cross table jnews_listmailings
	 * @param int $listID - list id
...
//			$senddate = jnews::getNow();
	...
	function getListMailingToDelete($listID){
		if( empty($listID) )
    		{
    			return false;
    		}
 ...
 */
	...
	function insertListMailings( $lsid){
		$db = &JFactory::getDBO();
...
/** we call this function to get the content of the articles via ajax
 * we directly insert the article in the newsletter without the tags
 * */	
function getContent( $artId = 0, $contentType = 0, $task ='', $template_id=''){
	$mainframe =& JFactory::getApplication();
	ob_clean();
...
function getContentDetail($id=0){
	$db=&JFactory::getDBO();
...
function getMailingTemplate($mailingId=''){
	$db=&JFactory::getDBO();
...
/***
 * function to update the senddate of the newsletter when sent
 * @param - $mailingid - id of the mailing to update the sendate
 */
function updatesenddate( $mailingid){
...
/**
	 ...
	 * @param - $type
	 */
	function countMails($type = 1, $smartCount = false ){
		static $count = array();
	 ...
	 */
	function replaceOldTagsToNew( $content ){
